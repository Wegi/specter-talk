...
(defn hit-card!
  ...
  [player-name data]
  (let [player (first (filter #(= player-name (:name %)) data))
        list-without-player (remove #(= player-name (:name %)) data)]
    (vec (conj list-without-player
               (update player :hand conj (random-bj-card!))))))
...
(defn hit-card-specter!
  [player-name data]
  (setval [ALL #(= player-name (:name %)) :hand END]
          [(random-bj-card!)]
          data))
...
(setval [ALL :hand ALL (pred= :king)] :ace blackjack-data)
...
;; What Heroes did the players use?
(comment
...
(defn unparse-sentence
  [words]
  (str/join " " words))

(def WORDS (parser parse-sentence unparse-sentence))


(comment
  (def polite-data
    (setval [ALL :chat ALL :key WORDS ALL (pred= "gl")] "Good Luck" dota-data))
  (select [ALL :chat ALL :key WORDS ALL] polite-data)
  )
...
(comment
  (apply +
         (select [ALL :teamfights ALL :players ALL :damage] dota-data))
  )
